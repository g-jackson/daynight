...
public class DSPixelPerfectCamera : MonoBehaviour
{
    ...
    void ValidateUserInput()
    {
        ...
        {
            pixelsPerUnit = 1;
            Debug.Log("Warning: Pixels-per-unit must be greater than zero. Resetting to minimum allowed.");
        }
        else if (verticalUnitsOnScreen <= 0)
        {
            verticalUnitsOnScreen = 1;
            Debug.Log("Warning: Units-on-screen must be greater than zero. Resetting to minimum allowed.");
        }
    }
}
